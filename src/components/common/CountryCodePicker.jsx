const COUNTRY_DIAL_CODES = [
  ...
  { name: 'US Virgin Islands', iso2: 'VI', dialCode: '+1-340' },
...
function CountryCodePicker({ value, onChange, className = '', id = 'country-code' }) {
  const selectedCountry =
    COUNTRY_DIAL_CODES.find((country) => country.dialCode === value) ||
    COUNTRY_DIAL_CODES[0]

  return (
    <div className={`relative ${className}`}>
      <div className="flex h-full items-center gap-1 px-2 text-sm font-medium text-slate-900">
        <span>{toFlagEmoji(selectedCountry.iso2)}</span>
        <span>{selectedCountry.dialCode}</span>
        <span className="ml-auto text-xs text-slate-500 pointer-events-none">
          <svg
          ...
          >
            <path
              fillRule="evenodd"
              d="M5.23 7.21a.75.75 0 0 1 1.06.02L10 11.086l3.71-3.854a.75.75 0 1 1 1.08 1.04l-4.25 4.417a.75.75 0 0 1-1.08 0L5.21 8.27a.75.75 0 0 1 .02-1.06Z"
              clipRule="evenodd"
            />
          </svg>
        </span>
      ...
      >
        {COUNTRY_DIAL_CODES.map((country) => (
          <option key={`${country.iso2}-${country.dialCode}`} value={country.dialCode}>
            {toFlagEmoji(country.iso2)} {country.name} ({country.dialCode})
          </option>
        ))}
      </select>
    </div>
